...
public class EarlyMixinsLoader implements IFMLLoadingPlugin, IEarlyMixinLoader {
    ...
    public String getMixinConfig() {
        ...
        return "mixins.aggressivepatch.early.json";
    }
    ...
    public List<String> getMixins(Set<String> loadedCoreMods) {
        ...
        if (NBTConfig.compressLevel != 24) {
            mixins.add("minecraft.MixinPacketBuffer");
        }

        // Whole-TCP-stream zstd, one-directional (server compresses, client decompresses).
        // Only the block codec is implemented so far; gated here so the injection point stays clean.
        if (NettyConfig.enabled) {
            mixins.add("minecraft.MixinNetworkManage");
        }
