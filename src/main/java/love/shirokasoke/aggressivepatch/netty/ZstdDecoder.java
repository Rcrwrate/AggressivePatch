 ...
 * - Zstd availability check inlined against com.github.luben.zstd
...
public final class ZstdDecoder extends ByteToMessageDecoder {
    ...
    private static final int DEFAULT_MAX_FORWARD_BYTES = 64 * 1024; // inlined CompressionUtil.DEFAULT_MAX_FORWARD_BYTES
    /**
    ...
    public ZstdDecoder(int maximumAllocationSize, int maxWindowLog) {
        if (maximumAllocationSize < 0) {
            throw new IllegalArgumentException("maximumAllocationSize: " + maximumAllocationSize + " (expected: >= 0)");
        }
        if (maxWindowLog < MIN_WINDOW_LOG || maxWindowLog > MAX_WINDOW_LOG) {
            throw new IllegalArgumentException(
                "maxWindowLog: " + maxWindowLog + " (expected: " + MIN_WINDOW_LOG + '-' + MAX_WINDOW_LOG + ')');
        }
        ...
        this.maxWindowLog = maxWindowLog;
    }
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        try {
            ...
            try {
                ...
                if (in.isDirect()) {
                    // Inlined CompressionUtil.safeNioBuffer(...)
                    ByteBuffer nioBuf = in.nioBufferCount() == 1
                        ? in.internalNioBuffer(in.readerIndex(), in.readableBytes())
                        : in.nioBuffer(in.readerIndex(), in.readableBytes());
                    uncompressedLength = Zstd.getFrameContentSize(nioBuf);
                } else {
                    uncompressedLength = Zstd
                        .getFrameContentSize(in.array(), in.readerIndex() + in.arrayOffset(), in.readableBytes());
                }
                ...
                do {
                    if (outBuffer == null) {
                        outBuffer = ctx.alloc()
                            .heapBuffer(
                                (int) (maximumAllocationSize == 0 ? uncompressedLength
                                    : Math.min(maximumAllocationSize, uncompressedLength)));
                    }
                    do {
    ...
    public void channelReadComplete(ChannelHandlerContext ctx) throws Exception {
        // Note: ByteToMessageDecoder in Netty 4.0 does not expose discardSomeReadBytes().
        // The 4.0 decoder already compacts the cumulation buffer after each channelRead(),
        // so explicit discarding here is unnecessary for compatibility.

        if (needsRead && !ctx.channel()
            .config()
            .isAutoRead()) {
            ctx.read();
        }
